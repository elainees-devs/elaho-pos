...
describe("Auth Routes", () => {
	...
	it("includes login, register, and register-business routes", () => {
		const children = authRoutes[0].children ?? [];
	...
	it("has a wildcard fallback redirecting to login", () => {
		const children = authRoutes[0].children ?? [];
		const wildcard = children.find((c) => c.path === "*");
		expect(wildcard).toBeDefined();
	});

	it("redirects index route to login", () => {
		const children = authRoutes[0].children ?? [];
		const indexRoute = children.find((c) => "index" in c && c.index);
		expect(indexRoute).toBeDefined();
	});

	it("all route constants are consistent with /auth prefix", () => {
